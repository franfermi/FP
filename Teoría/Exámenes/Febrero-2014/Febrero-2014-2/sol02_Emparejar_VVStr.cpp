...
vector <vector<string>> CreaEmparejamientos (vector <string> equipos)
{
	...
	for (int primero = 0; primero <num_elementos-1; primero++) {
		...
		for (int segundo = primero+1; segundo<num_elementos; segundo++) {
			...
			vector <string> pareja_nueva;
			pareja_nueva.push_back (primer_elemento);
			pareja_nueva.push_back (segundo_elemento);
...
int main()
{
	...
	vector <vector<string>> emparejamientos = CreaEmparejamientos (equipos);
	...
	for (int pos=0; pos<num_parejas; pos++) {
		...
		vector <string> una_pareja = emparejamientos.at(pos);

		cout << "("   << una_pareja.at(0) 
			 << ", "  << una_pareja.at(1) << ")" << endl;
	}
